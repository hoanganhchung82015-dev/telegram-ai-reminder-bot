...
# 2. Lấy API Token từ biến môi trường (Hoặc nhập trực tiếp để test trên máy)
...
# Khởi tạo Gemini Client & Scheduler
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = (
        ...
        "2. **Tóm tắt nội dung/link:** Paste văn bản hoặc link bài báo vào đây để tôi tóm tắt."
    )
    await update.message.reply_text(welcome_text, parse_mode="Markdown")

# Hàm gửi thông báo khi đến giờ
...
async def process_reminder_with_ai(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str):
    ...
    try:
        ...
        if data.get("is_reminder"):
            ...
            # Nếu không phải nhắc nhở -> Chuyển sang AI tóm tắt văn bản
            await summarize_text_with_ai(update, context, user_text)

    except Exception as e:
        logging.error(f"Lỗi AI: {e}")
        await update.message.reply_text("❌ Không thể xử lý tin nhắn. Bạn vui lòng thử lại câu lệnh khác nhé!")
...
async def summarize_text_with_ai(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        await update.message.reply_text("❌ Có lỗi khi tóm tắt văn bản.")

# Xử lý tất cả tin nhắn văn bản từ User
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_text = update.message.text
    await process_reminder_with_ai(update, context, user_text)

# Xử lý sự kiện khi bấm nút trên thông báo
